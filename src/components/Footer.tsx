...
export default function Footer() {
  ...
  return (
    <footer className="bg-secondary border-t border-border pt-20 pb-10 text-secondary-foreground font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 pb-16 border-b border-border/60">
          ...
          <div className="lg:col-span-2 space-y-6">
            <Link href="/" className="group flex items-center gap-2">
              <span className="w-8 h-8 rounded-none bg-primary flex items-center justify-center text-white font-semibold text-lg">A</span>
            ...
            <p className="text-sm text-muted-foreground leading-relaxed max-w-sm">
              Crafting premium architectural surfaces, interlocking clay roofing tiles, facing bricks, and landscape paving systems. Marrying high geological materiality with sustainable circular design principles.
            </p>
            {/* Social Links */}
            <div className="flex gap-4">
              {[
                ...
                <a
                  key={i}
                  href={link}
                  className="w-10 h-10 rounded-none border border-border bg-white hover:border-primary/40 hover:bg-primary/5 text-muted-foreground hover:text-primary flex items-center justify-center transition-all duration-300"
                >
          ...
          <div>
            ...
            <ul className="space-y-4">
              {productLinks.map((link) => (
                <li key={link.name}>
                  <Link href={link.href} className="text-sm text-secondary-foreground hover:text-primary transition-colors duration-200">
                    {link.name}
                  </Link>
                </li>
              ))}
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Subscribe to get technical bulletins and architectural inspiration.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="relative flex items-center">
              <input
                ...
                disabled={submitted}
                className="w-full bg-white border border-border rounded-none py-2.5 pl-3 pr-10 text-xs text-foreground placeholder-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all disabled:opacity-50"
              />
              <button
                type="submit"
                disabled={submitted}
        ...
        {/* Bottom Credits */}
        <div className="flex flex-col md:flex-row items-center justify-between pt-8 gap-4 text-xs text-muted-foreground">
          <p>© {new Date().getFullYear()} AURA Materials Platform. All rights reserved.</p>
